...
from siteCompleto import app, bancodedados, bcrypt
from siteCompleto.forms import FormCriarConta, FormLogin, FormEditarPerfil, FormCriarPost
from siteCompleto.models import Usuario, Post
...
def login():
    ...
    if form_login.validate_on_submit() and 'botao_submit_login' in request.form:
        ...
        if usuario and bcrypt.check_password_hash(usuario.senha, form_login.senha.data):
            ...
            flash(f'Login realizado com sucesso! Usuário: {form_login.email.data}', 'alert-success')
    ...
    if form_criarconta.validate_on_submit() and 'botao_submit_criarconta' in request.form:
        ...
        bancodedados.session.add(usuario)
        bancodedados.session.commit()
        flash(f'Conta criada com sucesso! Usuário: {form_criarconta.email.data}', 'alert-success')
        return redirect(url_for('home'))
...
def editar_perfil():
    ...
    if form.validate_on_submit():
        ...
        current_user.cursos = atualizar_conhecimentos(form)
        bancodedados.session.commit()
...
def excluir_post(post_id):
    post = Post.query.get(post_id)
    if current_user == post.Autor:
        bancodedados.session.delete(post)
        bancodedados.session.commit()
